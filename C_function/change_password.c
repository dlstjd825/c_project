...
// __declspec(dllexport) bool change_password(const char* id, const char* old_pw, const char* new_pw) {
...
//     if (index == -1 || strcmp(users[index].pw, old_pw) != 0) {
//         free(users);
//         return false;
//     }

//     // Correct usage of strcpy_s
//     strcpy_s(users[index].pw, sizeof(users[index].pw), new_pw);
